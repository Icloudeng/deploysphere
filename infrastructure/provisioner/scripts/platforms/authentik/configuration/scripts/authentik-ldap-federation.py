...
for value in (SESSION_HEADERS, AUTHENTIK_URL, PROVIDER_NAME, LDAP_SERVER_URL, LDAP_BASE_CN, LDAP_BIND_DN, LDAP_BIND_PASSWORD):
...
def get_source_ldap(name: str):
    name = slugify(name)
    response = requests.get(
        f"{AUTHENTIK_URL}/sources/all/?search={name}",
        headers=headers
    )
